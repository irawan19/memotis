...
class SuratController extends AdminCoreController
{
    ...
    public function edit($id_surats=0)
    {
        ...
        if(General::hakAkses($link_surat, 'edit') == 'true')
        {
            ...
            if(!empty($cek_surats))
            {
                $cek_disposisi = Surat_disposisi::join('surat_users','surat_users.id_surat_users','=','surat_disposisis.surat_users_id')
																				->where('surat_users.surats_id',$cek_surats->id_surats)
																				->count();
