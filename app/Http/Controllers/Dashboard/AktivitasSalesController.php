...
class AktivitasSalesController extends AdminCoreController
{
    ...
    public function index(Request $request)
    {
        ...
        if(General::hakAkses($link_aktivitas_sales,'lihat') == 'true')
        {
            ...
            $data['hasil_kata']            = trim((string) $request->input('cari_kata', ''));
            $data['hasil_bulan']          = $request->input('cari_bulan') ?? '';
            $data['hasil_tahun']          = $request->input('cari_tahun') ?? '';
            ...
            $bulan = (int) $data['hasil_bulan'];
            $tahun = (int) $data['hasil_tahun'];
            if ($bulan >= 1 && $bulan <= 12 && $tahun >= 2000 && $tahun <= 2100) {
                $query->whereMonth('aktivitas_sales.tanggal_aktivitas_sales', $bulan)
                    ->whereYear('aktivitas_sales.tanggal_aktivitas_sales', $tahun);
            }
    ...
    /**
     * Filter pencarian: nama user, company, segmentation, PIC, status, dll (case-insensitive).
     */
    private function applyAktivitasSalesSearch($query, $hasil_kata)
    {
        ...
        $query->where(function ($q) use ($search) {
            $q->whereRaw('LOWER(COALESCE(users.name, "")) LIKE LOWER(?)', [$search])                    // nama user
                ->orWhereRaw('LOWER(aktivitas_sales.nama_aktivitas_sales) LIKE LOWER(?)', [$search])     // company
                ->orWhereRaw('LOWER(COALESCE(master_segmentasi_sales.nama_segmentasi_sales, "")) LIKE LOWER(?)', [$search])  // segmentation
                ->orWhereRaw('LOWER(COALESCE(master_kegiatan_sales.nama_kegiatan_sales, "")) LIKE LOWER(?)', [$search])       // sales
                ->orWhereRaw('LOWER(COALESCE(master_status_sales.nama_status_sales, "")) LIKE LOWER(?)', [$search])           // status
                ->orWhereRaw('LOWER(COALESCE(master_project_sales.nama_project_sales, "")) LIKE LOWER(?)', [$search])           // project
                ->orWhereRaw('LOWER(aktivitas_sales.pic_aktivitas_sales) LIKE LOWER(?)', [$search])      // PIC
                ->orWhereRaw('LOWER(aktivitas_sales.kontak_personal_aktivitas_sales) LIKE LOWER(?)', [$search])  // contact
                ->orWhereRaw('LOWER(aktivitas_sales.alamat_aktivitas_sales) LIKE LOWER(?)', [$search])  // alamat
                ->orWhereRaw('LOWER(aktivitas_sales.catatan_aktivitas_sales) LIKE LOWER(?)', [$search]) // result/catatan
                ->orWhereRaw('LOWER(COALESCE(aktivitas_sales.jangka_waktu_aktivitas_sales, "")) LIKE LOWER(?)', [$search])
                ->orWhereRaw('LOWER(COALESCE(master_level_sistems.nama_level_sistems, "")) LIKE LOWER(?)', [$search])
                ->orWhereRaw('LOWER(COALESCE(master_divisis.nama_divisis, "")) LIKE LOWER(?)', [$search])
